...
async function seed() {
  ...
  for (const c of concepts) {
    ...
    if (existing.rows && existing.rows.length > 0) {
      ...
      skipped++;
      console.log(`Updated rubric (tiers): ${c.concept_key}`);
      continue;
    }
    ...
    inserted++;
    console.log(`Inserted: ${c.concept_key} (${c.name})`);
  }
  ...
  if (!existingPrompt.rows || existingPrompt.rows.length === 0) {
    ...
    console.log('Inserted gross prompt for ' + subject + ' / ' + topic);
  } else {
    console.log('Gross prompt already exists for ' + subject + ' / ' + topic);
  }
